...
exports.enableViewRouting = function (app, opts) {
    if(!opts || opts.noPartials === false)
        app.use(partials());
    var fallbackDevice = (opts && opts.fallbackDevice) ? opts.fallbackDevice : '';
    app.use(function (req, res, next) {
        ...
        res.render = function(name, options, fn) {
            ...
            if(ignore === false) {
                ...
                if (check(req, res, deviceView)) {
                    name = deviceView;
                } else if (!!fallbackDevice) {
                    name = path.join(fallbackDevice, name);
                }
            }
